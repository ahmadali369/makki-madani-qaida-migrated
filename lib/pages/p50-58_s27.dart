...
class P50_S28 extends StatefulWidget {
  const P50_S28({super.key});

  @override
  State<P50_S28> createState() => _P50_S28State();
}

class _P50_S28State extends State<P50_S28> {
  ...
  void initState() {
    super.initState();
    _buttonStates = List<bool>.filled(22, false);
    _audios = List.generate(
      22,
      (index) => 'audios/p58/${index + 1}.mp3',
    );
  }
  ...
  Future<void> _triggerButtonsSequentially() async {
   ...
   double adjustedLeft(int col) {
    col = 5 - col; // Adjusting column index for left alignment
  ...
  double adjustedTop(int row) {
    double base = 0.049; // top padding adjusted to fit the grid
    double spacing = 0.13; // space between rows
